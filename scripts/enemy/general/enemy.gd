...
var movement_service: EnemyMovementService
...
func _ready():
	...
	movement_service = EnemyMovementService.new(self)
...
func _physics_process(_delta: float):
	if not player or GameManager.active_game_state == GameManager.WAVE_END:
		queue_free()
		return
	...
	if not enemy_attack_service.is_dashing:
		movement_service.resolve_movement(data.movement_type)
...
func die():
	var material_scene = preload("res://scenes/material.tscn")
